...
class GajiMingguan extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'gaji:mingguan';
    ...
    public function handle()
    {
        //
        $date = Carbon::now();
        $dateBefore = Carbon::now()->subDays(7);
        ...
        $transaction->name             = "Penggajian Mingguan ".$dateBefore." -".$date;
        $transaction->transaction_code = Transactions::getSysCode();
        $transaction->transaction_date = date("Y-m-d");
        $transaction->save();

        // Kredit Akun Kas (total gaji pokok - total potongan absen - total potongan kasbon).
