                        ...
                        <div class="row">
                            ...
                            <div class="col-md-2">
                                <form action="{{url('checkout')}}" method="post" >
                                    {!!csrf_field()!!}
                                    <input type="hidden" name="id" value="{{$trans->id}}">
                                    @if(count($trans->detail) > 0)
                                        <button type="submit" class="btn btn-block btn-primary btn-lg out">Checkout</button>
                                    @else
                                        <button type="button" class="btn btn-block btn-primary btn-lg cekout">Checkout</button>
                                    @endif
                                </form>
                            </div>
                            @if(Auth::user()->is('cashier'))
                                <div class="col-md-2">
                                    @if(count($trans->detail) > 0)
                                        <a class="btn btn-block btn-primary btn-lg out" data-toggle="modal"
                                       data-target="#modalPay">Pay</a>
                                    @else
                                        <button type="button" class="btn btn-block btn-primary btn-lg cekout">Pay</button>
                                    @endif
                        ...
                        <div class="row">
                            <div class="col-md-2">
                                <label class="col-md-12 bg-block bg-primary bg-lg"
                                       style="font-size: 200%">{{ isset($table) ? $table->name : ""}}</label>
                            </div>
                            <div class="col-md-4">
                                <label class="col-md-12 bg-block bg-primary bg-lg"
                                       style="font-size: 200%">{{ $trans->sales->order_type }}</label>
                            </div>
                            <div class="col-md-6">
                                ...
                                <table id="item_table" class="table table-hover table-striped">
                                    ...
                                    @foreach ($menus as $menu)
                                        <tr>
                                            ...
                                            <td>{{($menu->discount / ($menu->total + $menu->discount)) * 100}}</td>
                                            <td>Rp. {{$menu->discount}}</td>
                                            <td>Rp. {{$menu->total}}</td>
                                        </tr>
                                    @endforeach
                                    {!! csrf_field() !!}
                                    <input type="hidden" id="table" value="{{ isset($table) ? $table->id : ""}}">
                        ...
                        <div class="row" style="font-size: 120%">
                            <div class="col-md-4">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-7"><label>:</label> {{ Auth::user()->name }}</div>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="row">
                                ...
                                <div class="row auth">
                                    ...
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            ...
                                            <input type="password" class="form-control" id="auth" name="auth"
                                            >
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            <button class="btn btn-info auth_code">Check</button>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="box-body">
                        <div class="row">
                            <div class="col-md-12" id="item">
                                @if($trans->sales->status != 'checkout')
                    ...
                    <form role="form" method="POST" action="{{ url('transaction/pay') }}">
                        <div class="form-group">
                            ...
                            <div>
                                <button type="button" data-type="cash" class="btn pay_type btn-success btn-lg cash"
                                        onclick="$('#byr').show('slow');$('#kmbl').show('slow');$('#emp').hide('slow');$('.adm').hide('slow');$('.mem').hide('slow')">
                                    Cash
                                </button>
                                <button type="button" data-type="credit" class="btn btn-info pay_type btn-lg"
                                        onclick="$('#byr').hide('slow');$('#kmbl').hide('slow');$('#emp').hide('slow');$('.adm').show('slow');$('.mem').hide('slow')">
                                    Credit
                                </button>
                                ...
                                <button type="button" data-type="postpone" class="btn btn-info btn-lg pay_type"
                                        onclick="$('#byr').hide('slow');$('#kmbl').hide('slow');$('#emp').show('slow');$('.adm').hide('slow');$('.mem').hide('slow')">
                                    Postpone
                                </button>
                                <button type="button" data-type="compliment" class="btn btn-info btn-lg pay_type"
                                        onclick="$('#byr').hide('slow');$('#kmbl').hide('slow');$('#emp').hide('slow');$('.adm').hide('slow');$('.mem').show('slow')">
                                    Compliment
                                </button>
                                <button type="button" data-type="void" class="btn btn-info btn-lg pay_type"
                                        onclick="$('#byr').hide('slow');$('#kmbl').hide('slow');$('#emp').hide('slow');$('.adm').hide('slow');$('.mem').hide('slow')">
                                    Void
                                </button>
                            </div>
                        </div>
                        ...
                        <div class="form-group adm">
                            ...
                            <select class="form-control banks input" name="bank_id">
                                <option disabled selected>Select ECD</option>
                        ...
                        <div class="form-group mem">
                            <label >Memo<a class="text-red">:</a></label>
                            <input  type="text" class="form-control input" placeholder="memo" name="memo">
                        </div>
                        <div class="form-group adm">
                        ...
                        @if(isset($deliv))
                            <div class="form-group">
                                <label>Biaya Delivery<a class="text-red">:</a></label>
                                <input type="number" class="form-control input" name="deliv"
                                       value="0">
                            </div>
                        @endif
        ...
        $(document).ready(function () {
            ...
            cRequest = $.ajax({
                ...
                success: function (data) {
                    ...
                    t_amount = parseInt(subst - t_discount) + parseInt($('#dlv').val());
            ...
            $('.cekout').click(function(){
                alert('Pilih atau save menu terlebih dahulu');
            });
            ...
            $('.auth_code').click(function () {
                ...
                if (manager && auth) {
                    var check = $.post('{{ url("manager/code")}}', {
                        _token: $('#token').val(),
                        manager: manager,
                        auth: auth,                    
                    });
                    ...
                    alert('Please select manager and input the code');
                }
...
//                        }
    ...
    //                console.log($(tid).val());
                ...
                alert('pilih menu terlebih dahulu untuk  save');
            }
        }
